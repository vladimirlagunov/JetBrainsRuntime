...
class UnixPath implements Path {
    ...
    public int compareTo(Path other) {
        var otherPath = ((UnixPath) other).path;
        int limit = Math.min(path.length, otherPath.length);
        int result = Arrays.compareUnsigned(path, 0, limit, otherPath, 0, limit);
        return result != 0 ? result : path.length - otherPath.length;
    }
